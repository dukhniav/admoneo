...
class Database:
        ...
        try:
            if not self.socketio_client.connected:
                self.socketio_client.connect(
                    "http://api:5123", namespaces=["/backend"])
    ...
    def log_scout(
        ...
        with self.db_session() as session:
            ...
            sh = ScoutHistory(pair, target_ratio,
                              current_coin_price, other_coin_price)
            session.add(sh)
            self.send_update(sh)
...
class TradeLog:
    ...
    def set_ordered(self, alt_starting_balance, crypto_starting_balance, alt_trade_amount, order_id):
        session: Session
        with self.db.db_session() as session:
