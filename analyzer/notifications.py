...
def authorized_only(command_handler: Callable[..., None]) -> Callable[..., Any]:
    ...
    def wrapper(self, *args, **kwargs):
        ...
        # start leads here
        chat_id = int(self._config.TGRAM_CHAT_ID)
        if cchat_id != chat_id:
            logger.info(
...
class Telegram(RPCHandler):
    ...
    def _init_keyboard(self) -> None:
        ...
        cust_keyboard = self._config.TGRAM_KEYBOARD
        if cust_keyboard:
    ...
    def _init(self) -> None:
        ...
        """
        self._updater = Updater(token=self._config.TGRAM_TOKEN, workers=0,
                                use_context=True)
        ...
        handles = [
            ...
            CommandHandler(['reload_config', 'reload_conf'],
                           self._reload_config),
        ...
        callbacks = [
            CallbackQueryHandler(self._status_table,
                                 pattern='update_status_table'),
            CallbackQueryHandler(self._daily, pattern='update_daily'),
            CallbackQueryHandler(self._profit, pattern='update_profit'),
            CallbackQueryHandler(self._balance, pattern='update_balance'),
            CallbackQueryHandler(
                self._performance, pattern='update_performance'),
    ...
    def compose_message(self, msg: Dict[str, Any], msg_type: CommsMsgType) -> str:

        # if msg_type == CommsMsgType.BUY:
        #     message = self._format_buy_msg(msg)

        # if msg_type in (CommsMsgType.BUY_CANCEL, CommsMsgType.SELL_CANCEL):
        #     msg['message_side'] = 'buy' if msg_type == CommsMsgType.BUY_CANCEL else 'sell'
        #     message = ("\N{WARNING SIGN} *{exchange}:* "
        #                "Cancelling open {message_side} Order for {pair} (#{trade_id}). "
        #                "Reason: {reason}.".format(**msg))

        # elif msg_type == CommsMsgType.BUY_FILL:
        #     message = ("\N{LARGE CIRCLE} *{exchange}:* "
        #                "Buy order for {pair} (#{trade_id}) filled "
        #                "for {open_rate}.".format(**msg))
        # elif msg_type == CommsMsgType.SELL_FILL:
        #     message = ("\N{LARGE CIRCLE} *{exchange}:* "
        #                "Sell order for {pair} (#{trade_id}) filled "
        #                "for {close_rate}.".format(**msg))
        # elif msg_type == CommsMsgType.SELL:
        #     message = self._format_sell_msg(msg)
        # elif msg_type == CommsMsgType.PROTECTION_TRIGGER:
        #     message = (
        #         "*Protection* triggered due to {reason}. "
        #         "`{pair}` will be locked until `{lock_end_time}`."
        #     ).format(**msg)
        # elif msg_type == CommsMsgType.PROTECTION_TRIGGER_GLOBAL:
        #     message = (
        #         "*Protection* triggered due to {reason}. "
        #         "*All pairs* will be locked until `{lock_end_time}`."
        #     ).format(**msg)
        if msg_type == CommsMsgType.STATUS:
            message = '*Status:* `{status}`'.format(**msg)

        elif msg_type == CommsMsgType.WARNING:
            message = '\N{WARNING SIGN} *Warning:* `{status}`'.format(**msg)

        elif msg_type == CommsMsgType.STARTUP:
            message = '{status}'.format(**msg)

        else:
            raise NotImplementedError(
                'Unknown message type: {}'.format(msg_type))
        return message
    ...
    def _status(self, update: Update, context: CallbackContext) -> None:
        ...
        #                 lines.append(
        #                     "*Open Order:* `{open_order}` - `{sell_order_status}`")
        #             else:
        ...
        logger.warning("Status button not implemented")
    ...
    def _status_table(self, update: Update, context: CallbackContext) -> None:
        ...
        #     """
        #     messages_count = max(
        #         int(len(statlist) / max_trades_per_msg + 0.99), 1)
        #     for i in range(0, messages_count):
        #         trades = statlist[i *
        #                           max_trades_per_msg:(i + 1) * max_trades_per_msg]
        ...
        #     self._send_msg(str(e))
        self._send_msg("Not implemented")
        logger.warning("Status table button not implemented")
    ...
    def _daily(self, update: Update, context: CallbackContext) -> None:
        ...
        #     self._send_msg(str(e))
        self._send_msg("Not implemented")
        logger.warning("Daily button not implemented")
    ...
    def _profit(self, update: Update, context: CallbackContext) -> None:
        ...
        #                query=update.callback_query)
        self._send_msg("Not implemented")
        logger.warning("Profit button not implemented")
    ...
    def _stats(self, update: Update, context: CallbackContext) -> None:
        ...
        self._send_msg("Not implemented")
        logger.warning("Stats button not implemented")
    ...
    def _balance(self, update: Update, context: CallbackContext) -> None:
        ...
        #     balance_dust_level = self._config['telegram'].get(
        #         'balance_dust_level', 0.0)
        #     if not balance_dust_level:
        #         balance_dust_level = DUST_PER_COIN.get(
        #             self._config['stake_currency'], 1.0)
        ...
        #     self._send_msg(str(e))
        self._send_msg("Not implemented")
        logger.warning("Balance button not implemented")
    ...
    def _forcesell(self, update: Update, context: CallbackContext) -> None:
        ...
        # trade_id = context.args[0] if context.args and len(
        #     context.args) > 0 else None
        # if not trade_id:
    ...
    def _forcebuy(self, update: Update, context: CallbackContext) -> None:
        ...
        #     pairs = [InlineKeyboardButton(
        #         text=pair, callback_data=pair) for pair in whitelist]

        #     self._send_msg(msg="Which pair?",
        #                    keyboard=self._layout_inline_keyboard(pairs))
        self._send_msg("Not implemented")
        logger.warning("Force buy button not implemented")
    ...
    def _trades(self, update: Update, context: CallbackContext) -> None:
        ...
        #     self._send_msg(str(e))
        self._send_msg("Not implemented")
        logger.warning("Trades button not implemented")
    ...
    def _delete_trade(self, update: Update, context: CallbackContext) -> None:
        ...
        #     self._send_msg(str(e))
        self._send_msg("Not implemented")
        logger.warning("Delete trades button not implemented")
    ...
    def _performance(self, update: Update, context: CallbackContext) -> None:
        ...
        #     self._send_msg(str(e))
        self._send_msg("Not implemented")
        logger.warning("Performance button not implemented")
    ...
    def _locks(self, update: Update, context: CallbackContext) -> None:
        ...
        #     self._send_msg(message, parse_mode=ParseMode.HTML)
        self._send_msg("Not implemented")
        logger.warning("Locks button not implemented")
    ...
    def _delete_locks(self, update: Update, context: CallbackContext) -> None:
        ...
        Returns the currently active locks
        """
        # arg = context.args[0] if context.args and len(
        #     context.args) > 0 else None
        # lockid = None
        # pair = None
        # if arg:
    ...
    def _edge(self, update: Update, context: CallbackContext) -> None:
        ...
        #         edge_pairs_tab = tabulate(
        #             chunk, headers='keys', tablefmt='simple')
    ...
    def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._analyzer.state)

        self._send_msg(
            f"*Log lifespan:* `{val['log_lifespan']}`\n"
            f"*Data exchange:* `{val['data_exchange']} \n"
            f"*Heartbeat interval:* `{val['heartbeat_interval']} \n"
            f"*Sleep time:* `{val['bot_sleep_time']} \n"
            f"*Check coin interval:* `{val['check_coins_interval']} \n"
            f"*Update coin interval:* `{val['update_coins_interval']} \n"
        ...
        if reload_able and self._config['telegram'].get('reload', True):
            ...
            if keyboard is not None:
                ...
                reply_markup = ReplyKeyboardMarkup(
                    self._keyboard, resize_keyboard=True)
        try:
            try:
                self._updater.bot.send_message(
                    self._config.TGRAM_CHAT_ID,
                    text=msg,
                    parse_mode=parse_mode,
                    reply_markup=reply_markup,
