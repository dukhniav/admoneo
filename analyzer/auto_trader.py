...
class AutoTrader:
    ...
    def transaction_through_bridge(self, pair: Pair, all_tickers: AllTickers):
        ...
        if balance and balance * from_coin_price > self.manager.get_min_notional(pair.from_coin, self.config.BRIDGE):
            can_sell = True
            self.logger.info("Trade profitable... approved")
        else:
            self.logger.info("Trade not profitable... skipping sell")

        if can_sell and self.manager.sell_alt(pair.from_coin, self.config.BRIDGE, all_tickers) is None:
            self.logger.info(
                "No buyers, going back to scouting mode...")
            return None
        ...
        self.logger.info(
            "Couldn't execute BUY order, going back to scouting mode...")
        return None
    ...
    def update_trade_threshold(self, coin: Coin, coin_price: float, all_tickers: AllTickers):
        ...
        if coin_price is None:
            self.logger.info("Shiiiiit... Skipping update... current coin {} not found".format(
                coin + self.config.BRIDGE, notification=False))
            return
        ...
        with self.db.db_session() as session:
            for pair in session.query(Pair).filter(Pair.to_coin == coin):
                ...
                if from_coin_price is None:
                    self.logger.info(
                        "{} not found. Skipping update...".format(
                            pair.from_coin + self.config.BRIDGE, notification=False), notification=False)
                    continue
        ...
        with self.db.db_session() as session:
            for pair in session.query(Pair).filter(Pair.ratio.is_(None)).all():
                if not pair.from_coin.enabled or not pair.to_coin.enabled:
                    continue
                self.logger.info(
                    f"Initializing {pair.from_coin} vs {pair.to_coin}", notification=False)

                try:
                    from_coin_price = all_tickers.get_price(
                        pair.from_coin + self.config.BRIDGE)
                    if from_coin_price is None:
                        #self.logger.info("Skipping initializing {}, symbol not found".format(pair.to_coin + self.config.BRIDGE), notification=False)
                        continue
                except:
                    continue
        ...
        for pair in self.db.get_pairs_from(coin):
            try:
                ...
                if optional_coin_price is None:
                    # logger.info(
                    # ping scouting... optional coin {} not found".format(
                    # to_coin + self.config.BRIDGE), notification=False)
                    continue
            except:
                continue
    ...
    def _jump_to_best_coin(self, coin: Coin, coin_price: float, all_tickers: AllTickers):
        ...
        if ratio_dict:
            ...
            self.logger.info(
                f"Scotty, beam me up! Will be jumping from {coin} to {best_pair.to_coin_id}")
        ...
        for coin in self.db.get_coins():
            ...
            if not any(v > 0 for v in ratio_dict.values()):
                ...
                if bridge_balance > self.manager.get_min_notional(coin.symbol, self.config.BRIDGE.symbol):
                    self.logger.info(
                        f"<@386289182579884043>: My liege! We shall be purchasing {coin} using the magnificent {self.config.BRIDGE.symbol} bridge")
