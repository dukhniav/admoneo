...
class BinanceAPIManager:
    def __init__(self, config: Config, db: Database):
        ...
        self.config = config
        self.enabled = False
        self._set_sell_attempt(True)
        self._set_buy_attempt(True)

    @cached(cache=TTLCache(maxsize=1, ttl=43200))
    def get_trade_fees(self) -> Dict[str, float]:
        # print({ticker["symbol"]: ticker["taker"] for ticker in self.binance_client.get_trade_fee()["tradeFee"]}
        #   )
    ...
    def retry(self, func, *args, **kwargs):
        ...
        while attempts < 20:
            try:
                ...
                if attempts == 0:
                    logger.info(
                        "Failed to execute order. Trying again...")
                    logger.info(e, notification=False)
                attempts += 1
        return None
    ...
    def wait_for_order(self, origin_symbol, target_symbol, order_id):
        while True:
            try:
                ...
                logger.info(
                    f"<@386289182579884043>: Some shit broke: {e}")
                time.sleep(1)
        ...
        while order_status["status"] != "FILLED":
            try:
                ...
                if self._should_cancel_order(order_status):
                    ...
                    logger.info(
                        "Order took too long to execute, canceled...")

                    # sell partially
                    if order_status["status"] == "PARTIALLY_FILLED" and order_status["side"] == "BUY":
                        logger.info(
                            "<@386289182579884043>: Sell partially filled amount")
                    ...
                    logger.info("Going back to look for some berries...")
                    return None

                if order_status["status"] == "CANCELED":
                    logger.info(
                        "Order is canceled, going back to look for some berries...")
                    return None
                ...
                logger.info(
                    f"<@386289182579884043>: Unexpected Error: {e}")
                time.sleep(1)
    ...
    def _buy_alt(self, origin_coin: Coin, target_coin: Coin, all_tickers):
        ...
        logger.info(
            f"<@386289182579884043>: Attempting to BUY {order_quantity} of {origin_symbol} @${from_coin_price}")
        ...
        while order is None:
            try:
                ...
                total = order_quantity*from_coin_price
                symbol = origin_symbol + target_symbol
                logger.info(
                    f"<@386289182579884043>: Pending order: {order_quantity} of {symbol} @ ${from_coin_price} | ${total}")
            except BinanceAPIException as e:
                logger.info(e)
                time.sleep(1)
            except Exception as e:  # pylint: disable=broad-except
                logger.info(f"<@386289182579884043>: Summ broke: {e}")

        trade_log.set_ordered(origin_balance, target_balance,
                              order_quantity, order["orderId"])
        ...
        total = order_quantity * from_coin_price
        logger.info(
            f"<@386289182579884043>: * BUY Executed: {order_quantity} of {origin_symbol} @ ${from_coin_price} | Total: ${total} *")
    ...
    def _set_sell_attempt(self, bool: Boolean):
        global attempt_sell
        attempt_sell = bool

    def _set_buy_attempt(self, bool: Boolean):
        global attempt_buy
        attempt_buy = bool
    ...
    def _sell_alt(self, origin_coin: Coin, target_coin: Coin, all_tickers: AllTickers):
        ...
        if attempt_sell:
            total = order_quantity * from_coin_price
            logger.info(
                f"<@386289182579884043>: Attempting to SELL {order_quantity}/{origin_balance} of {origin_symbol} @ ${from_coin_price} | Total: ${total}")
            attempt_sell = False
        ...
        total = order_quantity * from_coin_price
        logger.info(
            f"<@386289182579884043>: Current order: {order_quantity} of {origin_symbol + target_symbol} @ ${from_coin_price} | ${total}")

        trade_log.set_ordered(origin_balance, target_balance,
                              order_quantity, order["orderId"])

        # Binance server can take some time to save the order
        logger.info("Waiting, Binance is being slow")
        ...
        total = order_quantity * from_coin_price
        logger.info(
            f"<@386289182579884043>: * SELL executed: {order_quantity} of {origin_symbol} @ ${from_coin_price} | Total: ${total} *")
