...
# from analyzer.enums import SellType, State
...
# from analyzer.plugins.pairlist.pairlist_helpers import expand_pairlist
# from analyzer.rpc.fiat_convert import CryptoToFiatConverter
# from analyzer.strategy.interface import SellCheckTuple
...
class RPC:
    ...
    def __init__(self, analyzer) -> None:
        ...
        :param analyzer: Instance of a analyzer bot
        :return: None
        """
    ...
    def _rpc_show_config(config: Config, botstate: Union[State, str]) -> Dict[str, Any]:
        ...
        val = {
            'state': str(botstate),
            'telegram_enabled': config.TGRAM_ENABLED,
            'telegram_notifications': config.TGRAM_NOTI,
            'notification_status': config.TGRAM_STATUS ,
            'notification_warning': config.TGRAM_WARNING ,
            'notification_startup': config.TGRAM_STARTUP,
            'initial_state': config.INITIAL_STATE ,
            'data_exchange': config.EXCHANGE,
            'heartbeat_interval': config.HEARTBEAT_INTERVAL ,
            'bot_sleep_time': config.BOT_SLEEP_TIME ,
            'check_coins_interval': config.CHECK_COINS_INTERVAL ,
            'update_coins_interval': config.UPDATE_COINS_INTERVAL,
            'log_lifespan': config.LOG_LIFESPAN
        }
        return val
    #
    ...
    #                 order = self._analyzer.exchange.fetch_order(trade.open_order_id, trade.pair)
    #             # calculate profit and send message to user
    #             if trade.is_open:
    #                 try:
    #                     current_rate = self._analyzer.exchange.get_rate(
    ...
    #                     current_profit_abs,
    #                     self._analyzer.config['stake_currency'],
    #                     self._analyzer.config['fiat_display_currency']
    #                 )
    #
    ...
    #                 base_currency=self._analyzer.config['stake_currency'],
    ...
    #     starting_balance = self._analyzer.wallets.get_starting_balance()
    ...
    #     total = 0.0
    #     try:
    #         tickers = self._analyzer.exchange.get_tickers(cached=True)
    #     except (ExchangeError):
    #         raise RPCException('Error getting current tickers.')
    #
    #     self._analyzer.wallets.update(require_update=False)
    #     starting_capital = self._analyzer.wallets.get_starting_balance()
    ...
    #
    #     for coin, balance in self._analyzer.wallets.get_all_balances().items():
    ...
    #         else:
    #             try:
    #                 pair = self._analyzer.exchange.get_valid_pair_combination(coin, stake_currency)
    ...
    #         })
    #     if total == 0.0:
    #         if self._analyzer.config['dry_run']:
    ...
    #         'note': 'Simulated balances' if self._analyzer.config['dry_run'] else ''
    #     }
    ...
    def _rpc_start(self) -> Dict[str, str]:
        ...
        self._analyzer.state = State.RUNNING
        return {'status': 'starting trader ...'}

    def _rpc_stop(self) -> Dict[str, str]:
        """ Handler for stop """
        if self._analyzer.state == State.RUNNING:
            self._analyzer.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> Dict[str, str]:
        ...
        self._analyzer.state = State.RELOAD_CONFIG
    ...
    # #         self._analyzer.config['max_open_trades'] = 0
    # #
    ...
    #
    #             if order['side'] == 'buy':
    #                 fully_canceled = self._analyzer.handle_cancel_enter(
    #                     trade, order, CANCEL_REASON['FORCE_SELL'])
    #
    #             if order['side'] == 'sell':
    #                 # Cancel order - so it is placed anew with a fresh price.
    #                 self._analyzer.handle_cancel_exit(trade, order, CANCEL_REASON['FORCE_SELL'])
    #
    ...
    #             self._analyzer.execute_trade_exit(trade, current_rate, sell_reason)
    #     # ---- EOF def _exec_forcesell ----
    #
    #     if self._analyzer.state != State.RUNNING:
    #         raise RPCException('trader is not running')
    #
    #     with self._analyzer._exit_lock:
    #         if trade_id == 'all':
    #             # Execute sell for all open orders
    #             for trade in Trade.get_open_trades():
    #                 _exec_forcesell(trade)
    #             Trade.commit()
    #             self._analyzer.wallets.update()
    ...
    #     """
    #
    #     if not self._analyzer.config.get('forcebuy_enable', False):
    ...
    #     stake_currency = self._analyzer.config.get('stake_currency')
    #     if not self._analyzer.exchange.get_pair_quote_currency(pair) == stake_currency:
    #         raise RPCException(
    ...
    #
    #     # gen stake amount
    #     stakeamount = self._analyzer.wallets.get_trade_stake_amount(pair)
    #
    #     # execute buy
    #     if self._analyzer.execute_entry(pair, stakeamount, price, forcebuy=True):
    #         Trade.commit()
    #         trade = Trade.get_trades([Trade.is_open.is_(True), Trade.pair == pair]).first()
    #         return trade
    #     else:
    ...
    #         if trade.open_order_id:
    #             try:
    #                 self._analyzer.exchange.cancel_order(trade.open_order_id, trade.pair)
    #                 c_count += 1
    #             except (ExchangeError):
    #                 pass
    #
    #         # cancel stoploss on exchange ...
    #         if (self._analyzer.strategy.order_types.get('stoploss_on_exchange')
    #                 and trade.stoploss_order_id):
    #             try:
    #                 self._analyzer.exchange.cancel_stoploss_order(trade.stoploss_order_id,
    #                                                                trade.pair)
    #                 c_count += 1
    #             except (ExchangeError):
    #                 pass
    ...
    #     return {
    #         'current': len(trades),
    #         'max': (int(self._analyzer.config['max_open_trades'])
    #                 if self._analyzer.config['max_open_trades'] != float('inf') else -1),
    ...
    #     res = {'method': self._analyzer.pairlists.name_list,
    #            'length': len(self._analyzer.active_pair_whitelist),
    #            'whitelist': self._analyzer.active_pair_whitelist
    #            }
    #     return res
    #
    ...
    #     if add:
    #         for pair in add:
    #             if pair not in self._analyzer.pairlists.blacklist:
    #                 try:
    #                     expand_pairlist([pair], self._analyzer.exchange.get_markets().keys())
    #                     self._analyzer.pairlists.blacklist.append(pair)
    #
    #                 except ValueError:
    #                     errors[pair] = {
    ...
    #
    #     res = {'method': self._analyzer.pairlists.name_list,
    #            'length': len(self._analyzer.pairlists.blacklist),
    #            'blacklist': self._analyzer.pairlists.blacklist,
    #            'blacklist_expanded': self._analyzer.pairlists.expanded_blacklist,
    #            'errors': errors,
    #            }
    #     return res
    ...
    def _rpc_get_logs(limit: Optional[int]) -> Dict[str, Any]:
        ...
        records = [[datetime.fromtimestamp(r.created).strftime(constants.DATETIME_PRINT_FORMAT),
        ...
        #       "analyzer.worker", "INFO", "Starting worker develop"]

        return {'log_count': len(records), 'logs': records}

    # def _rpc_edge(self) -> List[Dict[str, Any]]:
    #     """ Returns information related to Edge """
    #     if not self._analyzer.edge:
    #         raise RPCException('Edge is not enabled.')
    #     return self._analyzer.edge.accepted_pairs()
    ...
    #     _data, last_analyzed = self._analyzer.dataprovider.get_analyzed_dataframe(
    #         pair, timeframe)
    #     _data = _data.copy()
    #     if limit:
    #         _data = _data.iloc[-limit:]
    #     return self._convert_dataframe_to_dict(self._analyzer.config['strategy'],
    ...
    #     from analyzer.data.dataprovider import DataProvider
    #     from analyzer.resolvers.strategy_resolver import StrategyResolver
    ...
    #     if (self._analyzer.strategy.plot_config and
    #             'subplots' not in self._analyzer.strategy.plot_config):
    #         self._analyzer.strategy.plot_config['subplots'] = {}
    #     return self._analyzer.strategy.plot_config
