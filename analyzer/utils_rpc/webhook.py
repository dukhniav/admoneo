...
from analyzer.utils.rpc import RPC, RPCHandler
...
class Webhook(RPCHandler):
    ...
    def send_msg(self, msg: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if msg['type'] == CommsMsgType.BUY:
                valuedict = self._config['webhook'].get('webhookbuy', None)
            elif msg['type'] == CommsMsgType.BUY_CANCEL:
                valuedict = self._config['webhook'].get(
                    'webhookbuycancel', None)
            elif msg['type'] == CommsMsgType.BUY_FILL:
                valuedict = self._config['webhook'].get('webhookbuyfill', None)
            elif msg['type'] == CommsMsgType.SELL:
                valuedict = self._config['webhook'].get('webhooksell', None)
            elif msg['type'] == CommsMsgType.SELL_FILL:
                valuedict = self._config['webhook'].get(
                    'webhooksellfill', None)
            elif msg['type'] == CommsMsgType.SELL_CANCEL:
                valuedict = self._config['webhook'].get(
                    'webhooksellcancel', None)
            elif msg['type'] in (CommsMsgType.STATUS,
                                 CommsMsgType.STARTUP,
                                 CommsMsgType.WARNING):
    ...
    def _send_msg(self, payload: dict) -> None:
        ...
        try:
            if self._format == 'form':
                ...
                raise NotImplementedError(
                    'Unknown format: {}'.format(self._format))
